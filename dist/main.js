...
const CatalogManager_1 = __webpack_require__(/*! ../services/CatalogManager */ "./src/services/CatalogManager.ts");
const DownloadManager_1 = __webpack_require__(/*! ../services/DownloadManager */ "./src/services/DownloadManager.ts");
const StorageManager_1 = __webpack_require__(/*! ../services/StorageManager */ "./src/services/StorageManager.ts");
const AppLauncher_1 = __webpack_require__(/*! ../services/AppLauncher */ "./src/services/AppLauncher.ts");
const sqlite_1 = __webpack_require__(/*! ../repositories/sqlite */ "./src/repositories/sqlite.ts");
const IntegrityChecker_1 = __webpack_require__(/*! ../services/IntegrityChecker */ "./src/services/IntegrityChecker.ts");
function setupIPC() {
    const appRepo = new sqlite_1.SQLiteApplicationRepository();
    const downloadedAppRepo = new sqlite_1.SQLiteDownloadedAppRepository();
    const prefRepo = new sqlite_1.SQLitePreferencesRepository();
    const catalogManager = new CatalogManager_1.CatalogManager(appRepo);
    const integrityChecker = new IntegrityChecker_1.IntegrityChecker();
    const storageManager = new StorageManager_1.StorageManager(downloadedAppRepo, prefRepo);
    const downloadManager = new DownloadManager_1.DownloadManager(storageManager, integrityChecker);
    const appLauncher = new AppLauncher_1.AppLauncher();
    // Catalog IPC
    electron_1.ipcMain.handle('catalog:fetch', async (event, page, pageSize) => {
        return await catalogManager.fetchCatalog(page, pageSize);
    });
    electron_1.ipcMain.handle('catalog:search', async (event, query) => {
        return await catalogManager.searchApplications(query);
    });
    // Download IPC
    electron_1.ipcMain.handle('download:start', async (event, appId, url) => {
        return await downloadManager.startDownload(appId, url);
    });
    electron_1.ipcMain.handle('download:cancel', async (event, appId) => {
    ...
    downloadManager.onDownloadComplete((appId) => {
        eventSender('download:complete', appId);
    });
    // Storage IPC
    electron_1.ipcMain.handle('storage:getDownloadedApps', async () => {
        return await storageManager.getDownloadedApps();
    });
    electron_1.ipcMain.handle('storage:deleteApp', async (event, appId) => {
        return await storageManager.deleteDownloadedApp(appId);
    });
    electron_1.ipcMain.handle('storage:getDiskUsage', async () => {
        return await storageManager.getDiskUsage();
    });
    // App Launcher IPC
    electron_1.ipcMain.handle('app:launch', async (event, appId, filePath) => {
...
const db_1 = __webpack_require__(/*! ../repositories/db */ "./src/repositories/db.ts");
...
const path = __importStar(__webpack_require__(/*! path */ "path"));
const electron_1 = __webpack_require__(/*! electron */ "electron");
const dbPath = path.join(electron_1.app.getPath('userData'), 'appstore.db');
...
function initDb() {
  ...
  `);
    ...
    const stmt = exports.db.prepare('SELECT COUNT(*) as count FROM preferences WHERE id = 1');
    const result = stmt.get();
    if (result.count === 0) {
        exports.db.prepare(`
      INSERT INTO preferences (id, downloadFolder)
      VALUES (1, ?)
    `).run(path.join(electron_1.app.getPath('downloads'), 'ElectronApps'));
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.SQLitePreferencesRepository = exports.SQLiteDownloadedAppRepository = exports.SQLiteApplicationRepository = void 0;
const db_1 = __webpack_require__(/*! ./db */ "./src/repositories/db.ts");
class SQLiteApplicationRepository {
    ...
    findById(id) {
        const row = db_1.db.prepare('SELECT * FROM applications WHERE id = ?').get(id);
        if (!row)
            return null;
        return {
            ...row,
    ...
    findAll() {
        const rows = db_1.db.prepare('SELECT * FROM applications').all();
        return rows.map(row => ({
            ...row,
            tags: JSON.parse(row.tags || '[]')
        }));
    ...
    delete(id) {
        db_1.db.prepare('DELETE FROM applications WHERE id = ?').run(id);
    }
    update(app) {
        this.save(app);
    }
}
exports.SQLiteApplicationRepository = SQLiteApplicationRepository;
class SQLiteDownloadedAppRepository {
    save(app) {
        const stmt = db_1.db.prepare(`
    ...
    findById(id) {
        const row = db_1.db.prepare('SELECT * FROM downloaded_apps WHERE id = ?').get(id);
        if (!row)
            return null;
        return {
            ...row,
    ...
    findAll() {
        const rows = db_1.db.prepare('SELECT * FROM downloaded_apps').all();
        return rows.map(row => ({
            ...row,
            isRunning: !!row.isRunning
        }));
    ...
    delete(id) {
        db_1.db.prepare('DELETE FROM downloaded_apps WHERE id = ?').run(id);
    }
    update(app) {
        this.save(app);
    }
}
exports.SQLiteDownloadedAppRepository = SQLiteDownloadedAppRepository;
class SQLitePreferencesRepository {
    ...
    load() {
        const row = db_1.db.prepare('SELECT * FROM preferences WHERE id = 1').get();
        return {
            ...row,
...
exports.SQLitePreferencesRepository = SQLitePreferencesRepository;
...
class AppLauncher extends events_1.EventEmitter {
    runningApps = new Map();
    async launchApp(appId, filePath) {
        if (this.runningApps.has(appId)) {
            throw new Error(`Application ${appId} is already running`);
        }
        const child = (0, child_process_1.spawn)(filePath, [], {
            detached: true,
            stdio: 'ignore'
        });
        child.unref();
...
class CatalogManager {
    ...
    async fetchCatalog(page, pageSize) {
        try {
            const response = await axios_1.default.get(`${this.catalogUrl}?page=${page}&pageSize=${pageSize}`);
    ...
    async getApplicationDetails(appId) {
        ...
        if (cachedApp)
            return cachedApp;
        try {
            const response = await axios_1.default.get(`https://username.github.io/electron-app-downloader/applications/${appId}.json`);
